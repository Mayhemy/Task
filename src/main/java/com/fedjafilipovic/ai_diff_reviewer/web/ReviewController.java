...
import com.fedjafilipovic.ai_diff_reviewer.service.JobService;
import com.fedjafilipovic.ai_diff_reviewer.web.ApiExceptions.InvalidJsonException;
import com.fedjafilipovic.ai_diff_reviewer.web.ApiExceptions.PayloadTooLargeException;
...
public class ReviewController {
    ...
    private String extractDiff(JsonNode root) {
        ...
        if (diffNode == null || diffNode.isNull()) {
            throw new com.fedjafilipovic.ai_diff_reviewer.diff.InvalidDiffException("diff is required");
        }
        if (!diffNode.isTextual()) {
            throw new com.fedjafilipovic.ai_diff_reviewer.diff.InvalidDiffException("diff must be a string");
        }
        String diff = diffNode.asText();
        if (diff.isBlank()) {
            throw new com.fedjafilipovic.ai_diff_reviewer.diff.InvalidDiffException("diff must not be empty");
        }
        return diff;
    }
